const { app, BrowserWindow, WebContentsView, session, Menu, shell, ipcMain, globalShortcut } = require('electron');
...
ipcMain.handle('hud:set-capture-excluded', (e, excluded) => {
  ...
  try { w.setContentProtection(!!excluded); } catch (_) {}
});
...
function _recorderApi(action, body) {
  ...
  const req = http.request({
    hostname: '127.0.0.1', port: RECORDER_PORT,
    path: '/api/recorder/' + action, method: 'POST',
...
function _recorderStatus(cb) {
  const http = require('http');
  http.get({ hostname: '127.0.0.1', port: RECORDER_PORT, path: '/api/recorder/status', timeout: 3000 },
    (res) => { let d = ''; res.on('data', c => d += c); res.on('end', () => { try { cb(JSON.parse(d)); } catch { cb(null); } }); }
  ).on('error', () => cb(null));
}
app.whenReady().then(() => {
  globalShortcut.register('Alt+R', () => {
    ...
    const http = require('http');
    http.get({ hostname: '127.0.0.1', port: RECORDER_PORT, path: '/api/recorder/screens', timeout: 3000 },
      (res) => {
        let d = '';
        res.on('data', c => d += c);
        res.on('end', () => {
          let screenIndex = 0;
          try { screenIndex = JSON.parse(d).last_screen_index || 0; } catch {}
