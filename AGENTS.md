...
Internal `main` → `public/main` (github.com/mkflyamd/aigator-releases) via squash-merge PR.

```powershell
.\tools\sync-to-public.ps1                 # full flow: squash, format, CI, open PR
.\tools\sync-to-public.ps1 -DryRun         # stop before pushing
.\tools\sync-to-public.ps1 -SkipTests      # skip pytest (faster)
```

The script automates the entire flow: squash-merges `main` onto `public/main`, resolves formatting-only conflicts (takes `main`'s version, re-runs Prettier/Black), normalizes `.secrets.baseline` paths, runs `uv sync --locked` + `pre-commit run --all-files` + `pytest tests -q`, force-pushes, opens a PR, and waits for CI.

**Manual fallback** (if the script needs adjustments mid-run):

1. `git checkout -B pr-public-vN public/main`
2. `git merge --squash main` — resolve conflicts by taking `main`'s version for all files
3. `uv run python tools/normalize_secrets_baseline.py` — fix baseline paths
4. `npx prettier --write "**/*.{js,css,html,md,yaml,yml,json}"` — match public formatting
5. `uv run black web/ tests/ tools/ tray/ packaging/`
6. `uv run pre-commit run --all-files` — auto-fix, stage, amend
7. `uv run pytest tests -q`
8. `git push public pr-public-vN:pr-public-vN --force`
9. `gh pr create --repo mkflyamd/aigator-releases --base main --head pr-public-vN ...`

**Conflict pattern**: public/main runs Prettier (JS/CSS/HTML) and Black (Python) on regions main also edited. public/main's side is always formatting-only — take main's version, then re-run the formatters to match the public baseline. Semantic changes from public/main (packaging fixes, test additions) need manual porting — check `git diff 5d4ef04..public/main -- <file>` for any real logic changes.

**`.secrets.baseline` cross-platform fix**: `tools/normalize_secrets_baseline.py` normalizes backslash paths to forward slashes and strips BOM/UTF-16 encoding. Run via pre-commit hook (auto) or standalone. Without this, detect-secrets fails on CI (Linux) because the baseline has Windows backslash paths.
