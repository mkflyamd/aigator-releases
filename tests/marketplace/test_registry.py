...
from marketplace.registry import (
    ...
    _fetch_anthropic_skill_ids,
    _load_anthropic_dir_cache,
    _save_anthropic_dir_cache,
...
SAMPLE_ENTRY = {
    "id": "powerbi",
    "name": "Power BI",
    "description": "Read Power BI reports",
    "version": "1.0.0",
    "tier": "Verified",
    "install_url": "https://example.com/powerbi.gator",
    "install_count": 100,
...
def test_dir_cache_skip_api_call_when_fresh(tmp_path, monkeypatch):
    ...
    with patch(
        ...
        side_effect=AssertionError("API call should be skipped when cache is fresh"),
    ):
...
def test_dir_cache_fetches_api_when_stale(tmp_path, monkeypatch):
    ...
    fake_response = [
        {"type": "dir", "name": "new-skill-1"},
        {"type": "file", "name": "README.md"},
        {"type": "dir", "name": "new-skill-2"},
    ]
...
def test_dir_cache_falls_back_to_stale_on_403(tmp_path, monkeypatch):
    ...
    with patch(
        "urllib.request.urlopen",
        side_effect=Exception("HTTP Error 403: rate limit exceeded"),
    ):
        ids, from_cache = _fetch_anthropic_skill_ids()
...
def test_dir_cache_no_cache_and_403_returns_empty(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "marketplace.registry._ANTHROPIC_DIR_CACHE_FILE", tmp_path / "nonexistent.json"
    )
    with patch(
        "urllib.request.urlopen",
        side_effect=Exception("HTTP Error 403: rate limit exceeded"),
...
def test_fetch_anthropic_skills_uses_cached_ids(monkeypatch, tmp_path):
    ...
    with patch(
        "urllib.request.urlopen",
        side_effect=AssertionError("API call should be skipped"),
    ):
