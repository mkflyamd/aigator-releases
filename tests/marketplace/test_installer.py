...
def test_install_skill_md_rejects_file_url(tmp_path, monkeypatch):
    ...
    importlib.reload(m)
    result = m.install_skill_md(
        "bad", "", "1.0", "Community", install_url="file:///etc/passwd"
    )
    assert result["ok"] is False
...
def test_install_github_folder_returns_orphans_when_no_resolution(
...
def test_install_github_folder_keeps_orphans_when_resolution_keep(
    monkeypatch, tmp_path
):
    import marketplace.installer as inst
...
def test_install_github_folder_deletes_orphans_when_resolution_delete(
    monkeypatch, tmp_path
):
    import marketplace.installer as inst
