...
def test_install_routes_claude_plugins_official_entry_to_plugin_installer():
    ...
    with patch("routes.marketplace.fetch_catalog", return_value=[_CPO_ENTRY]), \
         patch("marketplace.installer.install_claude_plugins_official_plugin",
               return_value={"ok": True, "plugin_id": "amd-skills", "skill_ids": ["amd-skills__a"]}) as mock_install, \
...
def test_install_without_consent_is_refused_and_returns_capabilities():
    ...
    caps = {"ok": True, "plugin_id": "amd-skills", "skill_count": 2,
            "has_mcp": False, "has_local_code": True, "mcp_servers": []}
    with patch("routes.marketplace.fetch_catalog", return_value=[_CPO_ENTRY]), \
         patch("marketplace.installer.get_claude_plugins_official_capabilities", return_value=caps), \
         patch("marketplace.installer.install_claude_plugins_official_plugin") as mock_install:
    ...
    assert body["capabilities"] == {
        "skill_count": 2, "has_mcp": False, "has_local_code": True, "mcp_servers": [],
    }
...
def test_install_with_consent_true_installs_and_threads_consented():
    with patch("routes.marketplace.fetch_catalog", return_value=[_CPO_ENTRY]), \
         patch("marketplace.installer.install_claude_plugins_official_plugin",
               return_value={"ok": True, "plugin_id": "amd-skills", "skill_ids": []}) as mock_install, \
         patch("routes.marketplace.load_installed_skill_prompts"):
        r = client.post("/api/marketplace/install",
                        json={"skill_id": "amd-skills", "consent": True})
    assert r.status_code == 200
...
def test_install_response_enriches_commands_from_registry():
    ...
    with patch("routes.marketplace.fetch_catalog", return_value=[_CPO_ENTRY]), \
         patch("marketplace.installer.install_claude_plugins_official_plugin",
               return_value={"ok": True, "plugin_id": "amd-skills", "command_ids": ["standup"]}), \
         patch("routes.marketplace.load_installed_skill_prompts"), \
         patch("routes.marketplace.COMMAND_REGISTRY",
               {"standup": {"body": "...", "description": "Daily standup template", "plugin_id": "amd-skills"}}):
        r = client.post("/api/marketplace/install",
                        json={"skill_id": "amd-skills", "consent": True})
    assert r.status_code == 200
...
def test_install_coding_hard_entry_refused_regardless_of_consent():
    ...
    with patch("routes.marketplace.fetch_catalog", return_value=[_CPO_LSP_ENTRY]), \
         patch("marketplace.installer.install_claude_plugins_official_plugin") as mock_install, \
         patch("marketplace.installer.get_claude_plugins_official_capabilities") as mock_caps:
        r = client.post("/api/marketplace/install",
                        json={"skill_id": "clangd-lsp", "consent": True})
...
def test_install_entry_missing_installable_key_is_refused():
    ...
    with patch("routes.marketplace.fetch_catalog", return_value=[entry_no_installable]), \
         patch("marketplace.installer.install_claude_plugins_official_plugin") as mock_install, \
         patch("marketplace.installer.get_claude_plugins_official_capabilities") as mock_caps:
        r = client.post("/api/marketplace/install",
                        json={"skill_id": "amd-skills", "consent": True})
...
def test_install_non_claude_plugins_official_entry_unaffected():
    ...
    with patch("routes.marketplace.fetch_catalog", return_value=[other_entry]), \
         patch("routes.marketplace.install_skill_md",
               return_value={"ok": True, "skill_id": "powerbi"}) as mock_legacy, \
         patch("routes.marketplace.load_installed_skill_prompts"), \
...
def test_preview_then_consent_install_real_state_handoff(tmp_path, monkeypatch):
    ...
    with patch.object(m.github_fetcher, "download_skill_tarball", return_value=files), \
        ...
        r2 = client.post(
            ...
            json={"skill_id": "amd-skills", "consent": True, "pinned_ref": resolved_ref},
        )
