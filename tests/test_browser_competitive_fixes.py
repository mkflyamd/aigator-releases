...
class TestFriendlyPlaywrightError:
    """Missing Playwright Chromium should become actionable guidance, not a raw stack message."""
    ...
    def test_missing_executable_maps_to_native_guidance(self):
        ...
        assert "Chrome / Edge" in msg
        assert "Settings" in msg
        assert "no download needed" in msg
...
class TestPlaywrightChromiumDetection:
    """playwright_chromium_installed reflects whether the binary is actually on disk."""
    ...
    def test_detects_installed_chromium(self, tmp_path, monkeypatch):
        ...
        assert browser_agent.playwright_chromium_installed() is True
