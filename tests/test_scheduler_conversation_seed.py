...
class TestRunTaskSeedsConversationStore:
    ...
    def test_run_task_seeds_conversation_when_context_id_set(
        ...
        async def _run():
            ...
            await tq._run_task(
                task_id, prompt, _make_run_fn(response), context_id=context_id
            )
        ...
        assert len(history) == 2, (
            f"Expected 2 turns (user+assistant), got {len(history)}: {history}"
        )
    ...
    def test_run_task_does_not_seed_when_no_context_id(self, tmp_path, monkeypatch):
        ...
        assert not new_keys, (
            f"conversation_store gained keys with no context_id: {new_keys}"
        )
