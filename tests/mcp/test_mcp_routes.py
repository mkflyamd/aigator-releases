...
def test_add_connection_missing_url():
    r = client.post(
        "/api/config/mcp", json={"url": "", "auth_type": "none", "auth_value": ""}
    )
...
def test_add_connection_success():
    with patch(
        "routes.mcp_routes.add_or_update",
        return_value={"ok": True, "id": "mcp-crm", "name": "CRM", "tool_count": 3},
    ):
        r = client.post(
            "/api/config/mcp",
            json={"url": "http://host/mcp", "auth_type": "none", "auth_value": ""},
        )
    assert r.status_code == 200
...
def test_delete_connection_not_found():
    with patch(
        "routes.mcp_routes.remove",
        return_value={"ok": False, "error": "Connection not found"},
    ):
...
def test_health_check():
    with patch(
        "routes.mcp_routes.health_check", return_value={"ok": True, "latency_ms": 42}
    ):
...
def test_add_connection_http_explicit_transport():
    ...
    with patch(
        "routes.mcp_routes.add_or_update",
        return_value={"ok": True, "id": "mcp-crm", "name": "CRM", "tool_count": 1},
    ):
        r = client.post(
            "/api/config/mcp",
            json={
...
def test_complete_secrets_not_found():
    with patch("routes.mcp_routes._load_connections", return_value=[]):
        r = client.post(
            "/api/config/mcp/plugin:ghost:server/complete-secrets", json={"values": {}}
        )
    assert r.status_code == 404
...
def test_complete_secrets_success():
    ...
    with (
        patch("routes.mcp_routes._load_connections", return_value=[pending]),
        patch(
            "routes.mcp_routes.complete_pending_secrets",
            return_value={
                "ok": True,
                "id": "plugin:dd-plugin:datadog",
                "tool_count": 2,
