...
def _post(raw_input, fetcher=None, llm=None):
    with (
        ...
        return client.post(
            "/api/config/mcp/analyze", json={"raw_input": raw_input}
        ).json()
...
def test_e2e_conductor_vscode():
    r = _post(
        '{"servers":{"conductor":{"url":"https://mcp-platform.amd.com/mcp/conductor_mcp"}}}'
    )
...
def test_e2e_multi_server_confidence_medium():
    r = _post(
        '{"mcpServers":{"a":{"command":"npx","args":["pkg-a"]},"b":{"command":"uvx","args":["pkg-b"]}}}'
    )
...
def test_e2e_github_url_with_mock():
    ...
    r = _post(
        "https://github.com/microsoft/playwright-mcp", fetcher=lambda u, llm=None: fake
    )
