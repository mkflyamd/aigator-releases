...
async def _execute_job(job_id: str) -> None:
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        async with db.execute(
            "SELECT prompt, skills FROM job_meta WHERE job_id = ?", (job_id,)
        ) as cur:
...
async def init_scheduler() -> None:
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        # Migration: add skills column if missing (existing DBs)
        try:
            await db.execute(_DDL_ADD_SKILLS_COL)
        except Exception:
...
async def add_job(
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute(
            ...
            "VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
            (job_id, name, prompt, trigger_type, json.dumps(trigger_args), token_budget, json.dumps(skills or []), now),
        )
        await db.commit()
...
async def run_job_now(job_id: str) -> str:
    ...
    task_id = await task_queue.enqueue(row["prompt"], skills=skills, context_id=job_id)
...
async def update_job(
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        await db.execute(
            "UPDATE job_meta SET name = ?, prompt = ?, trigger_type = ?, trigger_args = ? WHERE job_id = ?",
            (new_name, new_prompt, new_trigger_type, json.dumps(new_trigger_args), job_id),
        )
        await db.commit()
