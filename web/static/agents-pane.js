      ...
      mainBtn.addEventListener('click', async (e) => {
        ...
        try {
          ...
          if (t.context_id && typeof createTabWithId === 'function') {
            // Route into the job's stable context so follow-up messages share history
            if (_viewInNewTab) {
              createTabWithId(t.context_id, _tabTitle);
            } else {
              if (typeof switchTab === 'function' && typeof _tabs !== 'undefined' && _tabs.find(tb => tb.id === t.context_id)) {
                switchTab(t.context_id);
              } else {
                createTabWithId(t.context_id, _tabTitle);
              }
            }
          } else if (_viewInNewTab && typeof createTab === 'function') {
            createTab();
...
/* ── Schedule prompt helper ──────────────────────────── */

function _apStartSchedulePrompt(prefill = '') {
  if (typeof selectSkill === 'function') selectSkill('gator');
  const text = prefill || 'Schedule a task to ';
