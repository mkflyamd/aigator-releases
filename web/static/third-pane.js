...
function openThirdPane(type) {
  ...
  else if (type === 'email') title.innerHTML = _tpIcon('outlook') + 'Outlook';
...
async function _fetchTeamsList() {
  ...
  try {
    ...
    tpState._hasMore = data.has_more !== false;
    ...
    _setListCache('teams', tpState.list, { hasViewpoint: tpState._hasViewpoint, channels: tpState._channels });
    _dismissAuthOverlay();
    renderTeamsList(tpState.list);
    _startChatListPolling();
    _prewarmTeamsThreads(tpState.list);
...
function renderTeamsList(chats) {
  ...
  if (tpState._hasMore !== false) {
    const loadOlder = document.createElement('button');
    loadOlder.id = 'tp-teams-load-older';
    loadOlder.style.cssText = 'padding:.2rem .5rem;font-size:.7rem;border-radius:10px;border:1px solid var(--border,#334155);background:none;color:var(--text-sub,#94a3b8);cursor:pointer;white-space:nowrap;transition:all .15s';
    ...
    loadOlder.addEventListener('click', async () => {
      ...
      try {
        ...
        const res = await fetch(`/api/teams/chats?skip=${skip}&top=50`);
        if (!res.ok) throw new Error(`HTTP ${res.status}`);
        const data = await res.json();
        const more = data.chats || [];
        tpState._hasMore = data.has_more !== false;
        if (!more.length) { loadOlder.textContent = 'No more'; return; }
        const existing = new Set(tpState.list.map(c => c.id));
        const unique = more.filter(c => !existing.has(c.id));
        tpState.list.push(...unique);
        _clearListCache('teams');
        _setListCache('teams', tpState.list, { hasViewpoint: tpState._hasViewpoint, channels: tpState._channels });
        renderTeamsList(tpState.list);
      } catch {
        loadOlder.textContent = 'Retry';
  ...
  sections.forEach(section => {
    ...
    if (section.type === 'channel') {
      ...
      if (channelIdx > maxShow) {
        ...
        more.addEventListener('click', () => { tpState[_shownKey] += SECTION_PAGE_SIZE; renderTeamsList(tpState.list); });
      ...
      // Per-section "Load more" for chats
      if (section.items.length > maxShow) {
        const remaining = section.items.length - maxShow;
        const more = document.createElement('button');
        more.className = 'tp-load-more-btn tp-section-load-more';
...
function renderEmailList(messages, totalUnread) {
  const col = document.getElementById('tp-list-col');
  col.innerHTML = '';
  ...
  if (_addBtn) _addBtn.onclick = () => {
    ...
    if (isOpen) {
      ...
      else if (detailCol) detailCol.innerHTML = _gatorDetailHint('email');
    } else {
  ...
  requestAnimationFrame(() => scroll.focus());
...
async function _loadTeamsThread(chatId) {
  ...
  try {
    ...
    const payload = { messages: data.messages || [], chat: chatInfo, myId: data.my_id || '', peer_last_read: data.peer_last_read || '' };
...
function renderTeamsThread(messages, chat, myId, data) {
  data = data || {};
  const col = document.getElementById('tp-detail-col');
  col.innerHTML = '';
  ...
  // Also scroll when any images finish loading
...
function _renderReactionBar(barEl, reactions, chatId, msgId) {
  ...
  // Convert Skype codepoint-name keys like "2795_heavyplussign" → emoji char "✚"
...
function _startInlineEdit(textEl, msg, chatId) {
  ...
  const originalContent = msg.body_html || msg.body || '';
...
function _buildHtmlEditor({ placeholder, html }) {
  ...
  wrap.getHtml = () => editableDiv.innerHTML;
...
function _showNewTeamsCompose() {
  ...
  const header = document.createElement('div');
  header.className = 'tp-new-compose-header';
  header.textContent = 'New Conversation';
  wrapper.appendChild(header);
...
async function _fetchOneDriveFolder(folderId) {
  if (_odState.folderCache.has(folderId)) return _odState.folderCache.get(folderId);
  const res = await fetch(`/api/onedrive/folders?parent=${encodeURIComponent(folderId)}`);
  if (!res.ok) throw new Error((await res.json().catch(() => ({}))).detail || res.status);
  const data = await res.json();
  _odState.folderCache.set(folderId, data.items || []);
  return data.items || [];
}
...
function _odRenderFlatList(container, items, emptyMsg, title) {
  ...
  items.forEach(item => {
    ...
    flatActions.appendChild(_createPinBtn('onedrive', item.id, item.name, { file_path: item.path || item.name, web_url: item.web_url || '' }));
...
function renderOneDriveList(items) {
  ...
  allSorted.forEach(item => {
    ...
    actions.appendChild(_createPinBtn('onedrive', item.id, item.name, { file_path: item.path || item.name, size: item.size || 0, is_folder: !!item.is_folder }));
    if (item.web_url) {
...
async function _odCreateFolder(name, parentFolderId, inputRow, listCol) {
  ...
  try {
    const res = await fetch(`/api/onedrive/folders/${encodeURIComponent(parentFolderId)}`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
    ...
    _odState.folderCache.delete(parentFolderId);
...
function renderOneDriveFileDetail(item) {
  ...
  actionBar.appendChild(_createPinBtn('onedrive', item.id, item.name, { file_path: item.path || item.name, web_url: item.web_url || '' }));
...
function _odShowContextMenu(x, y, item) {
  ...
  pinBtn.addEventListener('click', () => {
    menu.remove();
    _togglePin('onedrive', String(item.id), item.name, { file_path: item.path || item.name, web_url: item.web_url || '' });
  });
...
function _odPromptRename(item) {
  ...
  async function doRename() {
    ...
    try {
      const res = await fetch(`/api/onedrive/items/${encodeURIComponent(item.id)}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ name: newName }),
      });
      if (!res.ok) throw new Error((await res.json().catch(() => ({}))).detail || res.status);
      _odState.folderCache.delete(_odState.selectedFolderId);
      overlay.remove();
      _odOpenSection(_odState.section);
    } catch (e) {
...
async function _odDeleteItem(item) {
  ...
  try {
    const res = await fetch(`/api/onedrive/items/${encodeURIComponent(item.id)}`, { method: 'DELETE' });
    if (!res.ok) throw new Error((await res.json().catch(() => ({}))).detail || res.status);
    _odState.folderCache.delete(_odState.selectedFolderId);
    tpState.selectedId = null;
...
async function _odUploadFile(file, folderId, queueEl, fileListEl) {
  ...
  try {
    ...
    _odState.folderCache.delete(folderId);
...
function _showNewEmailCompose(prefill) {
  ...
  const header = document.createElement('div');
  header.className = 'tp-new-compose-header';
  header.innerHTML = `<span>${prefill ? 'Draft Email' : 'New Email'}</span>`;
  wrapper.appendChild(header);
...
function renderEmailDetail(email) {
  ...
  if (email.meeting_message_type === 'meetingRequest') {
    ...
    const sendRsvp = async (response, label) => {
      ...
      try {
        const res = await fetch(`/api/calendar/events/${encodeURIComponent(email.event_id)}/respond`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
  ...
  // Inline reply area (hidden by default)
  const replyArea = document.createElement('div');
  replyArea.className = 'tp-reply-area hidden';
  replyArea.id = 'tp-reply-area';
  replyArea.innerHTML = `
    <div class="tp-reply-header">
      <span id="tp-reply-label">Reply to ${escapeHtml(email.from_name || '')}</span>
      <div style="display:flex;gap:.4rem;align-items:center">
        <label style="font-size:.72rem;color:var(--text-sub);display:flex;align-items:center;gap:.25rem;cursor:pointer">
          <input type="checkbox" id="tp-replyall-check" style="cursor:pointer"> Reply All
        </label>
        <button class="tp-ai-btn secondary" id="tp-reply-cancel" style="padding:.18rem .5rem">Cancel</button>
      </div>
    </div>
    <textarea class="tp-reply-input" id="tp-reply-input" placeholder="Write your reply…"></textarea>
    <div class="tp-reply-footer">
      <button class="btn-primary" id="tp-reply-send" style="font-size:.76rem;padding:.32rem .7rem">Send</button>
      <span id="tp-reply-msg" style="font-size:.74rem;color:var(--text-sub)"></span>
    </div>`;
  col.appendChild(replyArea);
  ...
  const showReply = (replyAll) => {
    replyArea.classList.remove('hidden');
    replyArea.querySelector('#tp-replyall-check').checked = replyAll;
    replyArea.querySelector('#tp-reply-label').textContent =
      replyAll ? 'Reply All' : `Reply to ${email.from_name || ''}`;
    replyArea.querySelector('#tp-reply-input').focus();
  };

  aiBar.querySelector('#tp-email-reply-btn').addEventListener('click', () => showReply(false));
  aiBar.querySelector('#tp-email-replyall-btn').addEventListener('click', () => showReply(true));
  aiBar.querySelector('#tp-email-forward-btn').addEventListener('click', () => {
    replyArea.classList.remove('hidden');
    replyArea.querySelector('#tp-reply-to-label').textContent = 'Forward to';
    const toField = replyArea.querySelector('#tp-reply-to');
    if (toField) { toField.value = ''; toField.readOnly = false; toField.focus(); }
    const subField = replyArea.querySelector('#tp-reply-subject');
    if (subField) subField.value = 'Fwd: ' + (email.subject || '');
    replyArea.dataset.forwardMode = 'true';
    replyArea.dataset.messageId = email.id;
  });

  // Wire @mention dropdown to reply textarea
  const replyTextarea = replyArea.querySelector('#tp-reply-input');
  if (replyTextarea) _wireMentionDropdown(replyTextarea, replyTextarea);

  replyArea.querySelector('#tp-reply-cancel').addEventListener('click', () => {
    replyArea.classList.add('hidden');
    replyArea.querySelector('#tp-reply-input').value = '';
  });

  replyArea.querySelector('#tp-reply-send').addEventListener('click', async () => {
    const body = replyArea.querySelector('#tp-reply-input').value.trim();
    const replyAll = replyArea.querySelector('#tp-replyall-check').checked;
    if (!body) return;
    const btn = replyArea.querySelector('#tp-reply-send');
    const msg = replyArea.querySelector('#tp-reply-msg');
    btn.disabled = true;
    btn.textContent = 'Sending…';
    try {
      const isForward = replyArea.dataset.forwardMode === 'true';
      const toField = replyArea.querySelector('#tp-reply-to');
      const endpoint = isForward ? '/api/email/forward' : '/api/email/reply';
      const payload = isForward
        ? { message_id: replyArea.dataset.messageId || email.id, to: toField?.value || '', comment: body }
        : { message_id: email.id, body, reply_all: replyAll };
      ...
      if (res.ok) {
        replyArea.classList.add('hidden');
        replyArea.querySelector('#tp-reply-input').value = '';
        replyArea.dataset.forwardMode = '';
        msg.textContent = '';
      } else {
        msg.textContent = 'Send failed — try again';
      }
    } catch {
...
document.addEventListener('keydown', e => {
  ...
  if (e.key === 'j' || e.key === 'ArrowDown') {
    ...
    const ri = document.getElementById('tp-reply-input');
    if (ri) {
      document.getElementById('tp-reply-area')?.classList.remove('hidden');
      ri.focus();
    }
  }
});
...
function _renderTeamsComposeForm(container, data) {
  ...
  textarea.className = 'tc-compose-textarea';
  textarea.rows = 8;
  ...
  // @mention and #channel dropdown support
  _wireMentionDropdown(textarea, msgWrap);
  ...
  sendBtn.addEventListener('click', async () => {
    ...
    try {
      ...
      // Extract @mentions from plain text BEFORE HTML conversion (avoids corrupting URLs/attributes)
      let mentionText = msg;
      const mentions = [];
      for (const p of recipients) {
        const name = p.name || p.email.split('@')[0];
        // Match @FullName first, then @FirstName — with word boundary to prevent @Jan matching @Janet
        const patterns = [name];
        if (name.includes(' ')) patterns.push(name.split(' ')[0]);
        // Sort longest first so full name is tried before first name
        patterns.sort((a, b) => b.length - a.length);
        for (const pat of patterns) {
          const escaped = pat.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
          const re = new RegExp('@' + escaped + '(?![\\w])', 'gi');
          const replaced = mentionText.replace(re, '\x00MENTION_' + mentions.length + '\x00');
          if (replaced !== mentionText) {
            mentionText = replaced;
            mentions.push({
              id: mentions.length,
              mentionText: name,
      ...
      // Convert to HTML, then replace placeholders with <span> mention tags
      let htmlMsg = _markdownToTeamsHtml(mentionText);
      for (let i = 0; i < mentions.length; i++) {
        htmlMsg = htmlMsg.replace('\x00MENTION_' + i + '\x00',
          '<span itemscope itemtype="http://schema.skype.com/Mention" itemid="' + i + '">' + mentions[i].mentionText + '</span>');
      }
...
function _initSlackPane() {
  ...
  // Wire toolbar search — searches threads in selected channel
  const _tpSearchInput = document.getElementById('tp-search-input');
  if (_tpSearchInput) {
    ...
    _tpSearchInput.oninput = () => {
      ...
      const q = _tpSearchInput.value.trim();
      if (!q) return;
      _slackSearchDebounce = setTimeout(() => {
        const ch = _slackState.selectedChannel;
        if (ch) _slackSearchThreads(ch, q);
        else tpInjectAIPrompt(`@slack Search for "${q}" across Slack channels`);
      }, 400);
    };
    _tpSearchInput.onkeydown = (e) => {
      if (e.key !== 'Enter') return;
...
function _cfBuildPageDetail(container, page, fallbackUrl) {
  ...
  // Body — sandboxed iframe for rich HTML
  if (page.body_html) {
    const frame = document.createElement('iframe');
...
function _renderConfluenceEditForm(container, data) {
  ...
  // Body
  const bodyField = _cfField('Body (HTML)');
