...
def get_fallback_provider() -> tuple["LLMProvider", str] | None:
    ...
    with _lock:
        ...
        if fb_profile is None:
            logger.warning("[fallback] fallback_profile_id=%r not found in config", fallback_id)
            return None

        # Reject self-referential fallback — the same provider that just stalled
        # will stall again.
        if fallback_id == _active_profile.get("id"):
            logger.warning("[fallback] fallback_profile_id points at the active profile — ignoring")
            return None

        # Resolve the fallback model
        fb_model = _active_profile.get("fallback_model", "") or fb_profile.get("active_model", "")
        if not fb_model:
        ...
        base_provider = "anthropic" if fb_profile.get("type") == "anthropic" else "openai"
        ...
        logger.info("[fallback] resolved fallback: profile=%r model=%r", fallback_id, fb_model)
