        ...
        def _producer() -> None:
            ...
            try:
                with self._client.messages.stream(**kwargs) as stream:
                    ...
                    # Some gateways sometimes return stop_reason="end_turn" even when the
                    # response contains tool_use blocks (it should be "tool_use"). This causes
                    # the agent loop to silently ignore tool calls and return early.
                    authoritative_stop_reason = stop_reason or "end_turn"
                    if tool_calls and authoritative_stop_reason != "tool_use":
                        import logging as _log
                        _log.warning(
                            "[provider] gateway returned stop_reason=%r but %d tool_call(s) present"
                            " — overriding to 'tool_use'",
                            authoritative_stop_reason, len(tool_calls),
                        )
                        authoritative_stop_reason = "tool_use"
