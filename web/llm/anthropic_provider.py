...
def _is_empty_text_block(blk) -> bool:
    """True for a text block whose text is empty/whitespace — the API rejects
    cache_control on these ('cache_control cannot be set for empty text blocks')."""
    return (
        isinstance(blk, dict)
...
def _cacheable_block_index(content: list) -> int | None:
    """Index of the last block in `content` that can carry a cache breakpoint,
    skipping empty text blocks. Returns None if no block qualifies."""
    for i in range(len(content) - 1, -1, -1):
        if not _is_empty_text_block(content[i]):
            return i
    return None
...
class AnthropicProvider(LLMProvider):
    ...
    async def stream_turn(
        ...
        if shared.PROMPT_CACHING_ENABLED and len(cached_messages) >= 2:
            ...
            if isinstance(content, str):
                # The API rejects cache_control on an empty text block, so only
                ...
                # empty text blocks the API would reject.
                target = _cacheable_block_index(content)
                if target is not None:
                    blk = dict(content[target])
        ...
        def _producer() -> None:
            ...
            try:
                with self._client.messages.stream(**kwargs) as stream:
                    ...
                    raw_content = [b for b in (self._block_to_dict(x) for x in final_msg.content) if b is not None]
    ...
    async def _fallback_non_streaming(self, kwargs: dict) -> AsyncIterator[StreamEvent]:
        ...
        raw_content = [b for b in (self._block_to_dict(x) for x in response.content) if b is not None]
        usage = {
