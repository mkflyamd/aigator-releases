...
def _clean_url(text: str) -> str:
    ...
    Users routinely paste URLs as `<https://...>` (markdown autolink) or
    `https://...,` / `https://...).` (sentence-fragment), and the trailing
    junk is sent to the server as part of the path, causing 404 / session
    terminated. Trim aggressively.
    """
    t = text.strip()
    # leading angle bracket from <https://...>
    if t.startswith("<"):
        t = t[1:]
    # trailing wrapper chars: > ) ] , . ; ' " and whitespace
    t = t.rstrip(">)],.;'\" \t\r\n")
    return t
