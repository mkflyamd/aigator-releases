...
async def _cached_onenote_fetch(key: str, fn, *args):
    ...
    if entry is not None:
        if entry.get("is_429"):
            raise HTTPException(
                status_code=429,
                detail="OneNote rate limit hit — please wait a moment and retry.",
            )
        return entry["data"]
...
async def context_unpin(source: str, item_id: str, context_id: str = "default"):
    ...
    if source == "onenote":
        ...
        to_remove = [
            k for k, v in pinned_onenote_pages.items() if v.get("page_id") == item_id
        ]
        for k in to_remove:
            del pinned_onenote_pages[k]
    return result
...
async def tp_onenote_sections(notebook_id: str):
    ...
    return await _cached_onenote_fetch(
        f"sections:{notebook_id}", _tool_list_onenote_sections, notebook_id
    )
...
async def tp_onenote_pages(section_id: str):
    ...
    return await _cached_onenote_fetch(
        f"pages:{section_id}", _tool_list_onenote_pages, section_id
    )
...
async def tp_onenote_page_content(page_id: str):
    ...
    def _fetch_page():
        ...
        _onenote_skills_dir = (
            Path(__file__).parent.parent / "skills" / "m365-onenote" / "scripts"
        )
        ...
        body_html = re.sub(
            r'src="(https://graph\.microsoft\.com/[^"]+)"', _rewrite_img, body_html
        )
...
async def tp_onenote_create_page(section_id: str, req: OneNoteCreatePageRequest):
    ...
    try:
        ...
        result = await asyncio.to_thread(
            _tool_create_onenote_page, section_id, req.title, req.body, req.html
        )
        return result
    except Exception as e:
        msg = str(e)
