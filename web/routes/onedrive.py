            ...
            "items": [
                {
                    ...
                    "modified": it.get("remoteItem", {}).get(
                        "lastModifiedDateTime", it.get("lastModifiedDateTime", "")
                    ),
                    "web_url": it.get("remoteItem", {}).get(
                        "webUrl", it.get("webUrl", "")
                    ),
                    "mime_type": it.get("remoteItem", {})
                    .get("file", {})
                    .get("mimeType", ""),
                    "drive_id": it.get("remoteItem", {})
                    .get("parentReference", {})
                    .get("driveId", ""),
                }
                for it in items
...
def onedrive_special_folder(folder_name: str):
    ...
    if folder_name not in allowed:
        raise HTTPException(
            status_code=400, detail=f"Unknown special folder: {folder_name}"
        )
    try:
        from skills._m365.helpers import get_graph_client
...
async def onedrive_upload_to_folder(folder_id: str, file: UploadFile):
    ...
    try:
        ...
        item = gc.put_binary(
            upload_path, content, file.content_type or "application/octet-stream"
        )
        item_id = item.get("id", "")
        if not item_id:
            raise HTTPException(
...
def onedrive_list_sites():
    ...
    try:
        ...
        data = gc.get(
            "/sites",
            params={"search": "*", "$select": "id,displayName,webUrl", "$top": "50"},
        )
        items = data.get("value", [])
        return {
            "sites": [
