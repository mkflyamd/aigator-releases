...
def tp_email_message(message_id: str):
    ...
    try:
        gc = GraphClient()
        # A pinned OWA item carries a conversationId, not a message id. Graph's
        # GET /me/messages/{id} 400s on that. Resolve it to the newest message in
        # the conversation once, up front, so every downstream call (beta fetch,
...
async def approve_draft(draft_id: str, body: dict = None):
    ...
    try:
        ...
        if dtype == "email-reply":
            ...
            # localhost port, which broke whenever the server ran on any other
...
async def open_draft_in_outlook(draft_id: str):
    ...
    that later GET/PATCH fails, the native draft still exists — releasing the
    claim back to "pending" via abort_handoff would let Approve deliver a
    second, independent message via Gator. So the except blocks below must
    not blindly abort: once msg_id has been assigned from a successful create
