...
async def tp_email_respond(message_id: str, req: MeetingRespondRequest):
    ...
    try:
        gc = GraphClient()
        gc.post(f"/me/events/{req.event_id}/{req.response}", {"sendResponse": req.send_response})
        return {"ok": True}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
def tp_email_reply(req: EmailReplyRequest):
    ...
    try:
        ...
        # Step 2: Set the body as HTML so formatting is preserved
        body_html = req.body
        if "<" not in body_html:
...
def tp_email_forward(req: EmailForwardRequest):
    ...
    try:
        ...
        draft_id = draft.get("id", "")
        # Step 2: Set recipients and body as HTML
        ...
        if req.comment:
            ...
            update["body"] = {"contentType": "HTML", "content": comment_html}
