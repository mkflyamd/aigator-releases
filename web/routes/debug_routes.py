...
async def get_conversation_turns(context_id: str, limit: int = Query(100, le=500)):
    ...
    return {"context_id": context_id, "turns": await turn_telemetry.get_conversation_timeline(context_id, limit)}


@router.get("/api/debug/turns", dependencies=[Depends(verify_csrf)])
async def get_turns_by_outcome(outcome: str = Query(...), limit: int = Query(50, le=500)):
    """Find all turns with a given outcome (e.g. 'stalled', 'llm_error',
    'circuit_breaker', 'max_iterations', 'cancelled')."""
    return {"outcome": outcome, "turns": await turn_telemetry.get_turns_by_outcome(outcome, limit)}
