...
def confluence_recent_pages():
    ...
    try:
        ...
        token = os.environ.get("CONFLUENCE_PAT", "") or os.environ.get(
            "ATLASSIAN_PAT", ""
        )
                        ...
                        pages.append(
                            {
                                ...
                                "last_modified": r.get(
                                    "lastSeen", r.get("friendlyLastModified", "")
                                ),
                                "last_modifier": "",
                            }
                        )
        ...
        params = urllib.parse.urlencode(
            {"cql": cql, "limit": 20, "expand": "space,version,history.lastUpdated"}
        )
...
def confluence_my_pages():
    ...
    try:
        ...
        try:
            ...
            params = urllib.parse.urlencode(
                {"cql": cql, "limit": 20, "expand": "space,version"}
            )
            data = confluence_api("GET", f"content/search?{params}")
        except Exception:
            pass
...
def confluence_space_pages(space_key: str):
    ...
    try:
        ...
        data = confluence_api(
            "GET",
            f"space/{space_key}/content/page?depth=root&limit=30&expand=version,children.page",
        )
        wiki = confluence_browse_url()
        pages = []
        for r in data.get("page", {}).get("results", data.get("results", [])):
            ...
            has_children = (
                children.get("size", 0) > 0 if isinstance(children, dict) else False
            )
            pages.append(
                {
                    "id": r.get("id", ""),
...
def confluence_page_detail(page_id: str):
    ...
    try:
        ...
        data = confluence_api(
            "GET",
            f"content/{page_id}?expand=body.storage,body.view,space,version,history.lastUpdated,children.page",
        )
        wiki = confluence_browse_url()
        ...
        children = [
            {
                "id": c.get("id", ""),
                "title": c.get("title", ""),
                "url": f"{wiki}{c.get('_links', {}).get('webui', '')}",
            }
...
def confluence_search(body: dict):
    ...
    try:
        ...
        if scope in ("all", "pages", "recent"):
            ...
            params = urllib.parse.urlencode(
                {"cql": cql, "limit": 15, "expand": "space,version,history.lastUpdated"}
            )
            data = confluence_api("GET", f"content/search?{params}")
            for r in data.get("results", []):
                history = r.get("history", {})
                ...
                pages.append(
                    {
                        "id": r.get("id", ""),
                        "title": r.get("title", ""),
                        "space": r.get("space", {}).get(
                            "name", r.get("resultGlobalContainer", {}).get("title", "")
                        ),
                        "url": f"{wiki}{r.get('_links', {}).get('webui', '')}",
            ...
            try:
                ...
                for s in space_data.get("results", []):
                    ...
                    if q_lower in name.lower() or q_lower in key.lower():
                        spaces.append(
                            {"key": key, "name": name, "type": s.get("type", "")}
                        )
            except Exception:
                pass
...
def confluence_create_page(body: dict):
    ...
    try:
        ...
        if not space_key or not title:
            raise HTTPException(
                status_code=400, detail="space_key and title are required"
            )
        payload: dict = {
            "type": "page",
            "title": title,
...
def confluence_update_page(page_id: str, body: dict):
    ...
    try:
        ...
        if err:
            parts = [
                "This page can't be saved — the content isn't balanced, so Confluence would reject it."
            ]
