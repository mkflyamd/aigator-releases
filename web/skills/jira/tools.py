...
TOOL_DEFS = [
    ...
    },
    {
        ...
        "input_schema": {"type": "object", "properties": {
            ...
            "comment": {"type": "string", "description": "Comment text. Use @accountId (Cloud) or @username (Server) from jira_search_user to mention someone."},
    ...
    },
    {
        ...
        "description": (
            ...
            "The base URL and auth are injected automatically — pass only the path after /rest/api/2/."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
                "path": {"type": "string", "description": "API path after /rest/api/2/ e.g. 'issue/ROCM-123/editmeta' or 'field'"},
    ...
    },
    {
        ...
        "input_schema": {
            ...
            "properties": {
                ...
                "path": {"type": "string", "description": "API path after /rest/api/2/ e.g. 'issue/ROCM-123'"},
...
def _jira_search_post(jql: str, max_results: int = 20, fields: list | None = None) -> dict:
    """Use POST /search/jql (required on Jira Cloud; falls back to POST /search for Server)."""
    if fields is None:
        fields = ["summary", "status", "priority"]
    try:
        # Jira Cloud: /rest/api/3/search/jql (old /search endpoint was removed)
        return jira_api("POST", "search/jql", {"jql": jql, "maxResults": max_results, "fields": fields}, api_version="3")
    except RuntimeError as e:
        if "410" in str(e) or "404" in str(e):
            # Jira Server fallback
            return jira_api("POST", "search", {"jql": jql, "maxResults": max_results, "fields": fields}, api_version="2")
        raise
...
def _build_adf_comment(text: str) -> dict:
    """Convert plain text with @accountId tokens to ADF for Jira Cloud comments."""
    # Split on @mention patterns — expect @<accountId> where accountId is alphanumeric/hyphen
    ...
    for part in parts:
        if part.startswith('@'):
            ...
            inline_nodes.append({
                "type": "mention",
                "attrs": {"id": account_id, "text": part}
            })
        elif part:
            inline_nodes.append({"type": "text", "text": part})
    return {
        "version": 1,
        "type": "doc",
...
def _tool_jira_get_epic_children(epic_key: str, max_results: int = 50) -> dict:
    jql = urllib.parse.quote(f'"Epic Link" = {epic_key} ORDER BY status ASC, priority DESC')
    data = jira_api("GET", f"search?jql={jql}&maxResults={max_results}&fields=summary,status,priority,assignee,issuetype")
...
def _tool_jira_get(path: str, query_params: dict | None = None) -> dict:
    ...
    try:
        ...
        # Return error verbatim — model needs the exact Jira message to self-correct
        return {"error": str(e)}
