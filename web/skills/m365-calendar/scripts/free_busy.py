...
def main() -> None:
    ...
    parser.add_argument(
        "--users", required=True, help="Comma-separated email addresses"
    )
    ...
    parser.add_argument(
        "--interval", type=int, default=30, help="Interval in minutes (default: 30)"
    )
    ...
    if args.json:
        ...
        for s in schedules:
            ...
            for slot in s["busy_slots"]:
                print(
                    f"    {slot['start']} - {slot['end']} [{slot['status']}] {slot['subject']}"
                )
            print()
