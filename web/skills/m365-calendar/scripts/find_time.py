...
def main() -> None:
    ...
    parser.add_argument(
        "--max", type=int, default=5, help="Max suggestions (default: 5)"
    )
    ...
    for slot in result.get("meetingTimeSuggestions", []):
        start = (
            slot.get("meetingTimeSlot", {}).get("start", {}).get("dateTime", "")[:16]
        )
    ...
    if args.json:
        print(
            json.dumps(
                {"total": len(suggestions), "suggestions": suggestions}, indent=2
            )
        )
    else:
        if not suggestions:
            ...
            return
        print(
            f"Available {args.duration}min slots on {date} with {', '.join(attendees)}:\n"
        )
