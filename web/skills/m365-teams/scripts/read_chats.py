...
def _save_skype_token(skype_token: str, messaging_service: str, expires_in: int) -> None:
...
def _exchange_skype_token(spaces_token: str) -> tuple[str, str]:
    ...
    messaging_service = d["regionGtms"]["chatService"] + "/v1"
    expires_in = d["tokens"].get("expiresIn", 86400)
    _save_skype_token(skype_token, messaging_service, expires_in)
...
def list_chats(skype_token: str, messaging_service: str, limit: int = 30) -> list[dict]:
    url = (
...
def read_messages(chat_id: str, skype_token: str, messaging_service: str, limit: int = 20) -> list[dict]:
    ...
    for msg in data.get("messages", []):
        ...
        # Parse reactions from properties.emotions (may be dict or JSON string)
...
def main() -> None:
    ...
    if args.chat_id:
        messages = read_messages(args.chat_id, skype_token, messaging_service, args.limit)
        if args.json:
        ...
        chats = list_chats(skype_token, messaging_service, args.limit)
        if args.json:
