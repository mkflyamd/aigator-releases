...
class GraphAPIError(RuntimeError):
    ...
    def __init__(
        self, message: str, status_code: int = 0, error_codes: list[int] | None = None
    ):
...
class GraphClient:
    ...
    def complete_auth(self, device_code: str) -> dict[str, str]:
        ...
        for _ in range(24):
            ...
            try:
                ...
                return {
                    "status": "error",
                    "error": f"{err}: {body.get('error_description', '')[:200]}",
                }
        return {
            "status": "error",
            "error": "Timed out waiting for user sign-in. Run auth again.",
        }
    ...
    def _headers(self) -> dict[str, str]:
        ...
        if not token:
            raise RuntimeError(
                "No valid access token — sign in via Settings → Apps → Microsoft 365."
            )
        return {
    ...
    def _extract_graph_error(self, status_code: int, body: str) -> GraphAPIError:
        ...
        try:
            ...
            error_codes = (
                err.get("error", {}).get("innerError", {}).get("error_codes", [])
            )
            if not error_codes:
        ...
        if status_code in (401, 403):
            ...
            if 65002 in error_codes:
                guidance = (
                    " Scope conflict — try re-authenticating with narrower scopes."
                )
            elif 70016 in error_codes:
                guidance = " Session expired — please sign in again via Settings."
            elif 50013 in error_codes:
                guidance = (
                    " Token not yet valid (clock skew) — wait a moment and retry."
                )
        ...
        return GraphAPIError(f"Graph API {status_code}: {msg}", status_code=status_code)
        ...
        *,
        ...
        for attempt in range(1, self._MAX_RETRIES + 1):
            try:
                ...
                log.log(
                    _lvl, "Graph %s %s -> %s", method, label or url, resp.status_code
                )
                return resp
                ...
                if code in (429, 502, 503, 504) and attempt < self._MAX_RETRIES:
                    ...
                    log.warning(
                        "Graph %d on %s %s — retrying in %ds (%d/%d)",
                        code,
                        method,
                        label,
                        wait,
                ...
                log.warning(
                    "Graph %s %s network error (attempt %d/%d): %s",
                    method,
                    label,
                    attempt,
                    self._MAX_RETRIES,
    ...
    def post(self, path: str, body: dict) -> Any:
        ...
        return (
            resp.json()
            if resp.content
            else {"status": "ok", "status_code": resp.status_code}
        )
    ...
    def batch(self, requests: list[dict]) -> list[dict]:
        ...
        for i in range(0, len(requests), 20):
            ...
            resp = self._request(
                "POST",
                url,
                content=json.dumps({"requests": chunk}).encode(),
                label="$batch",
            )
            data = resp.json()
