...
After every update_docx call, ALWAYS call read_docx or get_docx_info to verify the content was actually written. If the read returns an error, report the failure honestly. NEVER claim success without verification. Always tell the user which file was updated — state the **full absolute path** so the UI can render it as a clickable open-link.
...
```json
{
  ...
  "operations": [
    ...
    {
      "action": "insert_after",
      "find_text": "Introduction",
      "content_type": "paragraph",
      "text": "Added after intro."
    }
  ]
}
```
