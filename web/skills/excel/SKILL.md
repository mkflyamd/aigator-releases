...
When the user says "update / edit my workbook" and points at an existing file, you MUST ask — before the first write — whether to **overwrite the original in place** or **save a new copy** (and where). Do not decide this yourself. `update_excel` writes back to the `file_path` you pass, so pass the original path only after the user chose "overwrite" in this turn.
