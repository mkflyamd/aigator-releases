...
import shared
from config import OUTPUTS_DIR, INSTALLED_SKILLS_DIR
...
def _find_skill_dir(skill_id: str) -> Path | None:
    """Locate a skill's directory across the three install locations."""
    if not skill_id:
        return None
    candidates = [
...
_DELETE_FUNCS = {"rmtree", "rmdir", "remove", "unlink"}
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.Call):
            ...
            if isinstance(func, ast.Attribute):
                ...
                if func.attr in _DELETE_FUNCS:
                    blocked.append(f"Line {node.lineno}: .{func.attr}()")
