...
class MCPAdapter(ExtensionAdapter):
    ...
    def test_connection(self, config: dict) -> TestResult:
        ...
        if result.get("ok"):
            ...
            return TestResult(
                ok=True, detail=f"Found {n} tools", raw=result, tool_count=n
            )
        ...
        return TestResult(
            ok=False, detail=result.get("error") or "Connection failed", raw=result
        )
    ...
    def install(self, config: dict) -> InstallResult:
        ...
        if result.get("ok"):
            return InstallResult(
                ...
                name=result.get("name", config.get("name", "")),
            )
