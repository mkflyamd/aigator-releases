...
class TestSlackReactHelper:
    ...
    def test_slackReact_calls_both_endpoints(self):
        ...
        assert "/api/slack/unreact" in fn_body, (
            "_slackReact must call /api/slack/unreact"
        )
...
class TestSlackBuildReactionsRow:
    ...
    def test_reactions_row_makes_buttons(self):
        ...
        fn_body = JS_SRC[fn_start:fn_end]
        assert (
            "createElement('button')" in fn_body or 'createElement("button")' in fn_body
...
class TestNewCSSRules:
    ...
    def test_slack_reaction_btn_css_exists(self):
        assert ".slack-reaction-btn" in CSS_SRC, (
            ".slack-reaction-btn CSS rule must exist"
        )

    def test_slack_history_loading_css_exists(self):
        assert ".slack-history-loading" in CSS_SRC, (
            ".slack-history-loading CSS rule must exist"
        )
...
class TestMessageHistory:
    ...
    def test_slackState_has_cursor_fields(self):
        ...
        assert "_slackCursor" in state_body, (
            "_slackState must include _slackCursor field"
        )
        assert "_slackLoadingOlder" in state_body, (
            "_slackState must include _slackLoadingOlder field"
        )
    ...
    def test_load_older_uses_cursor(self):
        ...
        assert "_slackCursor" in fn_body, (
            "_slackLoadOlderMessages must use _slackState._slackCursor"
        )
    ...
    def test_load_older_preserves_scroll(self):
        ...
        assert "scrollHeight" in fn_body, (
            "_slackLoadOlderMessages must preserve scroll using scrollHeight diff"
        )
