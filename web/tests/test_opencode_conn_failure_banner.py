...
class TestConnFailureDetection:
    ...
    def test_scan_uses_a_rolling_buffer(self):
        ...
        assert ".slice(-" in body, (
            "the rolling buffer must be capped, not grow unbounded"
        )
...
class TestConnFailureBanner:
    ...
    def test_banner_css_exists_and_is_distinct_from_mcp_banner(self):
        assert ".oc-conn-banner" in CSS, (
            "connection-failure banner must have its own CSS"
        )
        ...
        assert shared_end != -1, (
            "must extend the shared banner structure, not duplicate it"
        )
