...
def _rf_region() -> str:
    ...
    assert start != -1, (
        "full-view _showReplyForwardCompose(mode, email) must exist (#1/#21)"
    )
...
class TestFullViewPane:
    def test_function_takes_a_mode(self):
        region = _rf_region()
        assert (
            "_showReplyForwardCompose(mode" in region
            or "function _showReplyForwardCompose(mode" in region
        ), (
...
class TestButtonsWireToFullView:
    def test_three_actions_open_full_view(self):
        ...
        assert "_showReplyForwardCompose('forward'" in TP_JS, (
            "Forward button must open full-view (#21)"
        )
        assert "_showReplyForwardCompose('reply'" in TP_JS, (
            "Reply button must open full-view (#21)"
        )
        assert "_showReplyForwardCompose('replyall'" in TP_JS, (
            "Reply All button must open full-view (#21)"
        )

    def test_send_targets_threaded_backends(self):
        region = _rf_region()
        assert "/api/email/reply" in region, (
            "reply/replyall must POST to the threaded reply backend (#1)"
        )
        assert "/api/email/forward" in region, (
            "forward must POST to the forward backend (#1)"
        )
...
class TestForwardBackendHonorsCcBcc:
    ...
    def test_forward_sets_cc_and_bcc_recipients(self):
        ...
        cc_addrs = [
            r["emailAddress"]["address"] for r in patch_update.get("ccRecipients", [])
        ]
        bcc_addrs = [
            r["emailAddress"]["address"] for r in patch_update.get("bccRecipients", [])
        ]
...
class TestReplyBackendHonorsRecipientOverrides:
    ...
    def test_reply_applies_to_override_when_provided(self):
        ...
        for call in gc.patch.call_args_list:
            upd = call.args[1]
            to_addrs += [
                x["emailAddress"]["address"] for x in upd.get("toRecipients", [])
            ]
            cc_addrs += [
                x["emailAddress"]["address"] for x in upd.get("ccRecipients", [])
            ]
        assert "new@x.com" in to_addrs, (
            "reply must apply the edited To recipients (#21)"
        )
    ...
    def test_reply_without_overrides_keeps_createreply_defaults(self):
        ...
        for call in gc.patch.call_args_list:
            upd = call.args[1]
            assert "toRecipients" not in upd, (
                "must not override recipients when none provided (#21)"
            )
