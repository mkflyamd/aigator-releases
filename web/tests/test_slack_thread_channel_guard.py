...
def test_read_thread_empty_channel_fails_loudly():
    ...
    with patch.object(tools, "is_slack_authenticated", return_value=True), \
         patch.object(tools, "_api") as mock_api:
        result = tools._handle_slack_read_thread(channel_id="", message_ts="1785265966.526289")
...
def test_read_thread_valid_channel_proceeds():
    ...
    with patch.object(tools, "is_slack_authenticated", return_value=True), \
         ...
         patch.object(tools, "_api", return_value={"ok": True, "messages": []}) as mock_api:
        tools._handle_slack_read_thread(channel_id="C07UXNA49RB", message_ts="1785265966.526289")
