...
def _idle_record(tmp_path, project_id="proj", owner=9000):
    rec = {
        "project_id": project_id, "repo_path": "r", "port": 8101,
        "pid": 0, "server_pid": 0, "password": "pw", "status": "running",
        "owner_port": owner,
...
class TestReaperActiveAttach:
    def test_skips_ready_server_with_live_attach(self, monkeypatch, tmp_path):
        ...
        im.reap_own_idle()
        assert killed == [], "a server with a live attached terminal must never be reaped"
        assert path.exists(), "its record must survive"
        refreshed = json.loads(path.read_text())["last_activity"]
        assert refreshed > rec["last_activity"], "last_activity must be refreshed on skip"
    ...
    def test_stuck_starting_server_reaped_even_with_attach(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(im, "active_attach_checker", lambda pid: True)
        im.reap_own_idle()
        assert len(killed) == 1, "a stuck-starting server is reaped regardless of attach claims"
...
class TestHasActiveAttach:
    ...
    def test_hook_exception_is_swallowed(self, monkeypatch):
        ...
        assert im._has_active_attach("x") is False, "a broken checker must not wedge the reaper"
...
class TestProjectHasLiveAttach:
    ...
    def test_all_dead_returns_false_and_drops_project(self, monkeypatch):
        ...
        assert "proj2" not in r._attach_by_project, "an all-dead project must be dropped"
...
class TestIsPtyViewed:
    def _mk(self, monkeypatch, alive=True, ws_clients=0, disconnected_ago=None):
        from routes import terminal
        import time as _t
        entry = {"done": False, "ws_clients": ws_clients,
                 "ws_disconnected_at": (None if disconnected_ago is None else _t.time() - disconnected_ago)}
        class _P:
            def isalive(self_inner): return alive
        entry["pty"] = _P()
