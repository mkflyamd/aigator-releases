...
def test_mention_regex_resolves_ids(monkeypatch):
    ...
    assert slack_tools._resolve_mentions_in_text(
        "Hi <@U0ARYKYUC67> can you let me in?"
...
def test_mention_with_pipe_form(monkeypatch):
    ...
    assert slack_tools._resolve_mentions_in_text("<@U0ARYKYUC67|taekmin> hi") == "@Taekmin Kim hi"


def test_no_mentions_untouched(monkeypatch):
    monkeypatch.setattr(slack_tools, "_resolve_user", lambda uid: "X")
    assert slack_tools._resolve_mentions_in_text("plain text, no mentions") == "plain text, no mentions"
    assert slack_tools._resolve_mentions_in_text("") == ""


def test_slackbot_mention(monkeypatch):
    monkeypatch.setattr(slack_tools, "_resolve_user", lambda uid: "should-not-be-called")
    assert slack_tools._resolve_mentions_in_text("<@USLACKBOT> reminder") == "@Slackbot reminder"
