...
SKILL = (
    pathlib.Path(__file__).parent.parent / "skills" / "aigator" / "SKILL.md"
...
def test_prompt_documents_run_python_write_capability():
    ...
    assert "write" in LOW
    assert "output_dir" in LOW, (
        "prompt must state HITL outside OUTPUT_DIR is expected, not a failure (#13)"
    )
