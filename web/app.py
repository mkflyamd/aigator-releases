...
# (e.g. AMD's network proxy) are honored without bundling a custom CA.
# Must run before any module that opens a TLS socket — that includes httpx,
...
def _load_skill_modules() -> None:
    ...
    if skills_pkg.exists():
        for entry in sorted(skills_pkg.iterdir()):
            ...
            # Annotate native tool descriptions with [Native] so the model can
            # prefer Verified/Community variants from the marketplace when both
            # exist. See aigator SKILL.md "Skill tier preference" rule.
            defs = [
                {**d, "description": f"[Native] {d.get('description', '')}".rstrip()}
                for d in defs
            ]
            shared.TOOL_TIER_MAP[skill_id] = "Native"
...
# (tool_name, arg) pairs where an empty string is a legitimate value for a
# required arg, so it must NOT be treated as a truncated/missing arg (#25):
# writing an empty file, or replacing matched text with nothing (deletion).
_EMPTY_OK_REQUIRED = {
    ("write_file", "content"),
    ("edit_file", "new_str"),
}
...
async def execute_tool(name: str, inputs: dict, *, context_id: str | None = None) -> dict:
    try:
        ...
        # (or has no annotation and no default — bare `def fn(args):`).
        # A function with one typed param like `def fn(page_id: str)` must be
        # called as fn(**inputs), not fn(inputs).
        ...
        if not single_dict_arg:
            ...
            # Guard against truncated tool-call arguments: if a large argument (e.g. a
            # 44KB Confluence body) blew the model's output budget, the tool-use JSON
            ...
            if missing:
                try:
                    payload_bytes = len(json.dumps(inputs, default=str).encode("utf-8"))
                except Exception:
                    payload_bytes = len(str(inputs))
                logging.getLogger(__name__).warning(
                ...
                return {
                    ...
                    "missing": missing,
                    "hint": (
                        "Required arguments are missing — they were likely truncated "
                        "because the payload exceeded the output budget. Reduce the "
